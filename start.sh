...
# Procura o app no lugar certo:
# tenta /workspace e /workspace/aplicativo (Railway pode montar assim)
FOUND_DIR=""
TARGET=""

for d in "/workspace" "/workspace/aplicativo" "." "./aplicativo"; do
  [ -d "$d" ] || continue
...
if [ -z "$TARGET" ]; then
  ...
  echo "[DEBUG] ls -la aplicativo:"
  ls -la aplicativo 2>/dev/null || true
  exit 1
fi
