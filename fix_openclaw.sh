...
openclaw config set gateway.auth.mode none 2>/dev/null || echo "Aviso: openclaw config não disponível ou falhou."
...
openclaw gateway install --force 2>/dev/null || echo "Aviso: 'gateway install' ignorado (ambiente sem systemd ou binário ausente)"

echo "3. Reiniciando Gateway..."
openclaw gateway restart 2>/dev/null || echo "Aviso: 'gateway restart' falhou."
...
if command -v openclaw >/dev/null 2>&1; then
  openclaw agent --agent main --message "oi" --timeout 30 2>&1 | grep -E "Waiting|Error|Failover" || echo "Teste concluído."
else
  echo "Binário 'openclaw' não encontrado no PATH para teste final."
fi
